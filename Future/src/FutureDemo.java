...
public class FutureDemo {
    public static void main(String[] args) throws InterruptedException, ExecutionException, TimeoutException {
        ...
        // 主线程会创建一个 Future 接口的对象，然后启动并发线程，并告诉并发线程，一旦你执行完毕，就把结果存储在这个 Future 对象里。
        ...
        // 二、实现了 Future 的 FutureTask
        // 见 class SquareCalculator
        ...
        while (!future.isDone()) {
            ...
            Thread.sleep(300); // InterruptedException
        }
        ...
        Integer timedResult = future.get(500, TimeUnit.MILLISECONDS); // TimeoutException
        // get(long, TimeUnit) 和 get() 的不同之处，是前者在经过指定的超时时间后任务仍未返回，那么就会抛出一个 TimeoutException 异常，表示执行超时。


        // 四、使用 Future.cancel() 方法取消 Future
        // 假设我们已经触发了一项任务，但由于某种原因，我们不再关心结果了。我们可以使用 Future.cancel(boolean) 告诉执行器停止操作并中断其底层线程。该方法很简单，使用演示如下
        Future<Integer> canceledFuture = new SquareCalculator().calculate(4);
        boolean canceled = future.cancel(true); // 不可中断
        ...
        System.out.println(result1 + " and " + future2);
        ...
        // 2 个任务是同时开始和结束运行的，整个过程大约需要 1 秒钟就能完成。
    }
}
